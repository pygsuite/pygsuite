...
def test_folder_creation(auth_test_clients):
    new_folder = Folder.create(title=f"Empty folder {TEST_ID}")
...
def test_folder_creation__nested(auth_test_clients, test_folder):
    ...
    new_nested_folder = Folder.create(
        title=f"Empty nested folder {TEST_ID}", parent_folder_ids=[parent_folder_id]
    )
...
def test_get_folder_files(auth_test_clients, test_folder):
    new_file_1 = File.create(
        title=f"Child file 1 {TEST_ID}", mimetype="text/plain", parent_folder_ids=[test_folder.id]
    )
    new_file_2 = File.create(
        title=f"Child file 2 {TEST_ID}", mimetype="text/plain", parent_folder_ids=[test_folder.id]
    )
