...
class TableRow(BaseElement):
    ...
    @property
    def cells(self):
        return [TableCell(val, self._presentation, table_id=self.table_id, column_index=idx, row_index=self.row_index)
...
class Table(BaseElement):
    ...
    @property
    def rows(self):
        return [TableRow(row, self._presentation, self.id, idx) for idx, row in enumerate(self._details['tableRows'])
                ]
